...
```{r echo=FALSE, out.width = "100%", fig.alt="Figure 1: DM+D data model"}
...
Amoxicillin 500mg capsules (A A H Pharmaceuticals Ltd)
...
-	Category C – these are drugs that are not readily available as generic. Category C prices are based upon one particular product, manufacturer, or sometimes supplier. Most of these drugs are preparation class 2 and vice-a-versa. 
...
Paracetamol_example_data <- data.frame(
  BNF_Name = "Paracetamol 500mg tablets",
  BNF_Code = "0407010H0AAAMAM",
  Preparation_Class = "01",
  Total_Items = "12,000,000"
)

# Displays the table using kable
knitr::kable(Paracetamol_example_data)
```
...
Eliquis_example_data <- data.frame(
...
# Display the table using kable
kable(Example_reimbursement_data)
...
# Display the table using kable
kable(example_preparation_class_data)
...
# Display the table using kable
kable(Example_preparation_class_2_data)
```
...
In the 2019 release of PCA we were unable to include the ‘Standard Quantity Unit’ field due to the completion of our ‘One Drug Database’ project that meant this data was no longer available to us. To meet user needs for this information we have included a new field within this release, ‘Unit of measure’. This field details the pharmaceutical form or unit that is used to measure a single unit of quantity of the given drug, appliance, or medical device. For example, tablet, capsule, millilitre (ml), grams (g), dose, unit dose, or vial. This list is not exhaustive, and many more units of measure can be found within the data.

### 3.2 Geographical breakdowns

We have also included geographic breakdowns within the data for Sustainability and Transformation Partnerships (STPs). STPs are health geographies that are formed by NHS organisations, local councils, and others to set out proposals to improve health and care for the local population. In these statistics the STP given is that of the dispensing contractor and not the prescribing organisation. These geographies are taken from NHSBSA administrative records and relate to the commissioning of services for NHS patients, and not the physical location of a dispensing contractor, or the patient that has received the drug, appliance, or medical device.
Some dispensing contractors are distance selling pharmacies or ‘online’ pharmacies. These contractors are required by regulation to provide a national service to NHS patients. However, they are only associated with a single STP in NHSBSA administrative records. In these cases, some STPs may have higher than expected dispensing of products or of certain drugs or appliances due to the existence of a distance selling or specialist pharmacy within their boundary.
In these statistics we have used the STP boundaries as of March 2021 and applied this throughout the year. Therefore, if a dispenser were to change STP throughout the year, the dispenser’s STP as of March 2021 would be used as their STP for the entire period. For data relating to periods before the creation of STPs in April 2020 predecessor organisations such as NHS England Area Teams may appear. This is caused by dispensing contractors that closed prior to April 2020 and were not mapped to the new organisations.
...
Not all products that are dispensed are assigned a BNF code and are instead classified in these statistics as an ‘unspecified item’. In 2020 150,000 items were classified as unspecified, 0.01% of all prescription items dispensed.
...
-	Providing breakdowns for more granular geographies than STP. 
...
In line with principle Q2.5 – Sound methods, within the [Code of Practice of Statistics](https://code.statisticsauthority.gov.uk/the-code/) and our Revisions and Corrections policy we are releasing data from 2014 to 2020 with this publication to maintain as consistent a time series as possible for users after the changes in methodology have been applied to these statistics.
...
The versions of PCA released by the other devolved administrations of the UK can be found their respective websites:
-	Northern Ireland – [Business Services Organisation](http://www.hscbusiness.hscni.net/services/3177.htm)
-	Scotland – [Public Health Scotland](https://beta.isdscotland.org/find-publications-and-data/health-services/pharmacy-and-prescribing/dispenser-payments-and-prescription-cost-analysis/)
-	Wales – [Welsh Government](https://nwssp.nhs.wales/ourservices/primary-care-services/general-information/data-and-publications/prescription-cost-analysis/)

### 6.2 Comparisons over time

We have provided the full back series of data that we have available to us at the NHSBSA from 2014 onwards to allow comparisons over time to be made more easily. Data from a previous series of PCA released by NHS Digital is [available from 2004 to 2018.](https://digital.nhs.uk/data-and-information/publications/statistical/prescription-cost-analysis)
...
The most granular data released in these statistics are also available via an API in the NHSBSA Open Data Portal.
...
STPs are health geographies that are formed by NHS organisations, local councils, and others to set out proposals to improve health and care for the local population. STPs replaced NHS England Local Offices and Area Teams on 1 April 2020.
**Total Quantity**
...
A [Continuous feedback survey](https://online1.snapsurveys.com/interview/caf0d685-0500-48d2-8b96-1c434a956819) is available on the Prescription Cost Analysis web page that can be completed by users.
