...
ref_data$ni_pca <- data_extract_NI("Y:\\Official Stats\\PCA\\data\\PCA_NI_202021.xlsx")

ref_data$scot_pca <- pca_import_scotland()

ref_data$wales_pca <- data_extract_wales("Y:\\Official Stats\\PCA\\data\\PCA_Wales_202021.xlsx")
...
ref_data$stp_pop <- ons_stp_pop()

ref_data$stp_lookup <- vroom("Y:/Official Stats/PCA/CCG_to_STP_lookup_arp_21.csv")
...
stp_pop <- ref_data$stp_pop %>%
  left_join(ods_stp_lookup,
            by = c("STP21_CD" = "STP21CDH")) %>%
  group_by(STP21_NM, STP21CDH, STP21_CD) %>%
...
map_df_raw <- bind_rows(
  # STP
  sf::read_sf(
    "https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Sustainability_and_Transformation_Partnerships_April_2021_EN_BUC_v2/FeatureServer/0/query?where=1%3D1&outFields=STP21CD,STP21NM&outSR=27700&f=json"
  ) %>%
...
dev_nations_data <- data.frame(
  ...
  "TOTAL_ITEMS" = c(
    add_anl_1 %>%
      filter(
        `Financial Year` == max_data_fy_minus_1
      ) %>%
      select(`Total Items`) %>%
      pull(),
    ref_data$wales_pca %>%
