pca_item_cost_BNF_sect <- function(con) {
  ...
  raw_data <- raw_data |>
    dplyr::mutate(RANK = row_number(desc(TOTAL_NIC)),
                  FILTER_YEAR = as.numeric(substr(YEAR_DESC,1,4))) |>
    dplyr::ungroup()  |>
    dplyr::filter(
  ...
  table <- data |>
    tidyr::pivot_wider(
      ...
      values_from = c(RANK,TOTAL_ITEMS,TOTAL_NIC,NIC_PER_ITEM)
    ) |>
